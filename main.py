...
def finetune(args, finetune_dataset, test_loader, model, criterion):
    ...
    for epoch in range(args.finetune_epochs):
        if args.world_size > 1:
            labeled_loader.sampler.set_epoch(epoch + 30)
...
def main():
    ...
    labeled_dataset, unlabeled_dataset, valid_dataset, test_dataset, finetune_dataset = DATASET_GETTERS[args.dataset](args)
    ...
    teacher_model = EfficientNet.from_pretrained(args.arch)
    student_model = EfficientNet.from_pretrained(args.arch)
...
if __name__ == '__main__':
    main()
    # args = parser.parse_args()
    # mp.spawn(main, nprocs=2, args = (2, args))
